...
| A lane that finished its turn (done or idle) without a receipt | The driver asks once. On the lane's reply (often its report, or a `herdr_complete` failure) or after 10 minutes, it sends one pointed ask: call `herdr_complete` now, as the only action, with the outcome and blockers from the last message. Ten minutes later it records a receipt from the lane's final report, marked inferred: its last message after the ask, or else its visible screen. That receipt goes through the normal stage handling (build to review, a verdict, integration lines), and any blocker lines are quoted to the root. With no report at all, the lane gets a fresh retry. An idle integrate lane whose commits are already on `spec-integration` is recorded as integrated. Past the first ask interval, a finished lane without a receipt holds no `maxParallel` slot. Every spec objective ends with the receipt rule. |
...
| A lane that never starts because of the infrastructure or harness (startup attestation, a shell that never came up, the bridge, a dirty worktree at dispatch) | Never counted toward exhausting the stage: the lane never ran, so its work and profile were never the problem. The driver retries after a growing wait (1 minute, doubling, at most 30), on the same profile. After 5 in a row the root gets one alert, and retries go on. A lane that delivers a receipt resets the count. Items a stage exhausted only through infrastructure errors (under older drivers) go back to that stage once per driver version. |
| An item marked done without a demo | `spec.defaults.evidence` (`{ "report": "artifacts/{id}.docx", "minImages": N }`) gives every item without its own evidence a required feature demo. Build and verify lanes are told to produce it with `demo-report.mjs`: a recorder for their Playwright run takes one screenshot per navigation or action (page visit, click, fill, submit, resulting state) and writes a Word document of ordered steps (a caption with the step number, the action and what it shows, then the image), plus `<report>.steps.json`. The verifier reads `word/document.xml` (stored or deflated, with no zip library) and fails a report whose screenshots lack a caption beside them, that has fewer screenshots than recorded steps, or that has no steps manifest. |
