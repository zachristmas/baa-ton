...
| A known-safe step inside a longer command (greps, builds, `cd`) | Segments are split only outside quotes. In `bypassPermissions` mode the hook checks only the segments that match the session's Bash `ask` rules (user and project settings), because in that mode nothing else can have caused the prompt. Other modes still classify the whole command, and any command substitution still defers. |
...
| A lane idle without a receipt (no question on screen) | The spec driver asks it once for its receipt. A status reply pushes the next ask back an hour (doubling). With no reply within 30 minutes, the item is blocked and handed to the root with the lane named. |
