...
The full reference that used to live in the README: every install path, updates and state migration, setup re-runs, profiles, harness support, and operating guarantees. Start with the [README](../README.md).
...
`npm run test:extension` and `npm run test:controller` run the packages separately. Live harness qualification is tracked in the [progress log](native-prerequisite-progress.md).
