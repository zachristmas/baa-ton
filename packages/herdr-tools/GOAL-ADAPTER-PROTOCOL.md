...
- **Idle-only delivery (unchanged):**
  ...
  - Only `agent_settled` with `ctx.isIdle()` and the matching active run may persist idle. The one exception is `session_start` with `reason: "reload"` while `ctx.isIdle()` is true: a `/reload` runs between turns, so there is no run to settle, and the reloaded root is idle. Without this, a root reloaded by a deploy stayed `unknown`, and the supervisor never nudged it. `tool_execution_end`, `turn_end`, `agent_end` and Herdr `pane.agent_status_changed` hooks do **not** grant idle authority, and silence never expires an active run into idle.
...
- **New goals:** `initialize`, including the goal created after a `reset`, starts the supervisor `running` with the first nudge one interval out. It keeps the previous goal's interval. It starts `paused` (with the previous `pauseReason`) only when the previous goal was explicitly paused; a `stop` is not carried over.
- **Stopped supervisor:** the controller alerts once per episode (Herdr notification plus a `supervisor-stopped` digest alert) when a goal that is neither completed nor paused has had its supervisor `stopped` for `watchdog_minutes`. `herdr_doctor` reports the same state as a `parent-goal-supervisor` warning. Neither restarts the supervisor.
