...
## Claude lanes: permission prompts without a person

A Claude lane runs with the operator's own settings underneath the generated ones, so an operator `ask` rule such as `Bash(rm *)` or `Bash(git checkout *)` still prompts inside a lane, and nobody is at a dispatched lane to answer. The generated settings add a `PermissionRequest` hook (`known-safe-hook.mjs`) that answers those prompts for commands `known-safe.mjs` accepts: removing temp files the same command created, a `mktemp -d` directory, one file in a session scratchpad, a relative output directory the command recreates, self-test directories, and creating or detaching a feature branch at `origin/main`. Every other segment of the command must be inert (read-only, or confined to creating files in the working tree or a scratchpad), or the hook stays silent and the prompt stays up. Every other prompt is routed to the root instead of blocking: the hook opens `herdr_request kind=permission` (carrying the tool name and full input) through the lane's own MCP bridge. An acknowledged approvalPolicy answers it at once where it matches. Otherwise the request reaches the root digest (the supervisor wakes the root for it even while Herdr reports the lane as working, since the hook holds the call), and the hook waits up to 10 minutes: a root grant allows the call, a root deny refuses it with the root's note, and no answer leaves Claude's normal prompt. Tools that need the person's own answer (`AskUserQuestion`, `ExitPlanMode`) are never routed. Every decision is logged to `.baa-ton/herdr-orchestrator/known-safe-approvals.jsonl`.

This replaces `--permission-prompt-tool` as the default route: that flag only applies to non-interactive (`-p`) runs, and dispatched lanes run interactively. The opt-in `BAA_CLAUDE_PERMISSION_PROMPT_TOOL=1` path is unchanged for print-mode use.
