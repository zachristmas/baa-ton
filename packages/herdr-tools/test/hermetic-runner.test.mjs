...
test("hermetic environment drops agent-session identity and keeps the rest", () => {
  ...
  assert.deepEqual(Object.keys(env).sort(), ["BAATON_OPERATOR_STORE", "BAA_TON_NO_RUNTIME_RECORDS", "CLAUDE_CONFIG_DIR", "HOME", "PATH"]);
  assert.match(env.BAATON_OPERATOR_STORE, /baa-hermetic-operator-/, "never the user's real operator store");
